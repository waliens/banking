...
def train_model(session, data_source="belfius", required_sample_size=50, random_state=42, class_level="fine"):
  ...
  model_file = MLModelFile(filename=model_filename, metadata_={"target": data_source, "class_level": class_level}, state=MLModelState.TRAINING)
  ...
  try:
    ...
    logging.getLogger().info("finish tuning model '{}' for target '{}' (cv_score={}, best_params={})".format(
      model_file.filename, model_file.metadata_["target"], gsearch.best_score_, gsearch.best_params_))
